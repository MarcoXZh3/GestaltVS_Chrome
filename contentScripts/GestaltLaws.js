...
function normailizedDistance_AtoB(nodeA, nodeB) {
  var leftA = nodeA.getAttribute('left'), leftB = nodeB.getAttribute('left');
  var topA = nodeA.getAttribute('top'), topB = nodeB.getAttribute('top');
  var rightA = nodeA.getAttribute('right'), rightB = nodeB.getAttribute('right');
  var bottomA = nodeA.getAttribute('bottom'), bottomB = nodeB.getAttribute('bottom');
  if (!leftA || !leftB)
    return null;
...
function sameImage(imgURL1, imgURL2, callback) {
  ...
  getImageFromUrl(imgURL1, function(img) {
    ...
    getImageFromUrl(imgURL2, function(img) {
      var img2 = img;
      // compare SSIM of the two images here
 ...
 * @param node                  the node list (visual tree node)
...
function splitByAllLaws(nodes) {
  ...
  for (var i = 0; i < nodes.length - 1; i++) {
    ...
    var same = (node1.getAttribute('css_position') == node2.getAttribute('css_position'));      // Common Fate
    if (!same)
 ...
 */
/*
function applyAllLaws(node, mergingResults) {
  if (node.childElementCount == 0)
    return ;

  var nhds = [], sames = [];
  for (var i = 0; i < node.childElementCount; i++) {
    applyAllLaws(node.children[i], mergingResults);
    if (i == node.childElementCount - 1)
      continue ;
    var child1 = node.children[i], child2 = node.children[i + 1];
    var same = (child1.getAttribute('css_position') == child2.getAttribute('css_position'));    // Common Fate
    if (!same)
      same = (child1.getAttribute('left') == child2.getAttribute('left') ||                     // Continuity
    ...
    sames.push(same);
    nhds.push(normalizedHausdorffDistance(child1, child2));                                     // Proximity
  } // for (var i = 0; i < node.childElementCount; i++)

  // Check all laws
  if(nhds.length != node.childElementCount - 1)
    console.log('error: ' + node.getAttribute('xpath'));
  if (nhds.length != 0) {
    var avg = 0.0;
    ...
    avg /= nhds.length;
    for (var i = 0; i < nhds.length; i++)
      if (sames[i] || nhds[i] <= avg)
        mergeResults(mergingResults, node.children[i], node.children[i+1]);
  } // if (nhds.length != 0)
  else
    mergeResults(mergingResults, node.children[0]);
